...
素材库现在是和 `解析 / 技术标 / 商务标 / 审计 / 设置` 同级的一级准备模块。进入解析和项目流程前，先在素材库维护 `通用素材 / 客户素材 / 项目素材` 和对应 Wiki；确认参与投标并补全项目时，系统会绑定本项目可读取的客户素材与项目素材范围。
...
- 项目确认后，缺口处理和生成标书默认读取 `通用素材/{标书类型}`、`客户素材/{客户}/{标书类型}`、`项目素材/{素材项目ID}/{标书类型}`，不会把其他客户或其他项目的资料混入搜索范围
