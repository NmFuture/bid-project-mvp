...
商务标入口 -> 商务标页面 -> 商务标 API -> 商务标 service -> 商务标 Skill -> 商务标素材/Wiki -> 商务标文档/编辑
```
...
商务标当前按独立工作区推进：解析、目录、商务缺口、共创编辑和审计都走商务标 API；商务标主流程不再把技术标的 `/generate`、`/coverage`、`/export` 页面作为必经步骤。
