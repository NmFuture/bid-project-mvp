...
- **opencode 是唯一的 LLM 出口**（`OPENCODE_BASE_URL=http://opencode:4096`，默认模型 `deepseek-v4-flash`（`big-pickle` 自动映射回退），超时 1800s）。所有 AI 能力都封装为 opencode Skill（`bid-tech-*` / `bid-business-*` / `bid-material-*`，位于 `code/sewpg-bid-backend/opencode/skills/`）。
