...
> **复验（2026-08-13，fix commit `486d16e`）：已修复。** `_request_slot` 改为 `acquire(blocking=False)` + `asyncio.sleep(0.1)` 非阻塞轮询：取消只会在 sleep 的 await 点投递，彼时协程不持有许可；acquire 成功到进入 `try/finally` 之间无 await 点，不存在「持有许可但 finally 未安装」的窗口。不换 `asyncio.Semaphore` 的理由（循环亲和性 vs 引擎实例跨事件循环复用）成立，预算原语与取值保持、留 B4 统一治理符合任务边界。新增回归测试 `test_request_slot_cancelled_while_waiting_does_not_leak_permit` 有区分度（旧 to_thread 实现下会失败）。复跑 2276 passed（基线 2274 + 新增 2），0 失败。详见文末「复验记录」。
...
### F3. P3 · non-blocking — `test_idle_timeout_aborts_session_and_joins_worker` 中线程回收断言已成死断言（**复验：已修复于 `486d16e`**）
...
### F4. P3 · non-blocking — 两条新关键路径无直接测试覆盖（**复验：已补测于 `486d16e`**）
...
## 复验记录（fix commit `486d16e`，2026-08-13）

- 改动范围：`opencode_engine.py`（`_request_slot` 11 行）+ `tests/test_opencode_engine.py`（48 行），无超范围改动。
- **F1 已修复**：非阻塞轮询获取消除泄漏窗口（见 F1 内联复验注记）；跨线程/跨循环语义保持（`threading.BoundedSemaphore` 非阻塞 acquire 在任意线程/循环安全），获取延迟至多 +0.1s，相对 HTTP 请求时长可忽略，行为保持不受影响。
- **F4 已处理**：新增 `test_request_slot_cancelled_while_waiting_does_not_leak_permit`（取消落等待窗口不泄漏许可，旧实现下会失败，有区分度）与 `test_polling_propagates_worker_error_to_caller`（worker 异常如实抛给主协程）。
- **F3 已处理**：`test_idle_timeout_aborts_session_and_joins_worker` 的线程死断言改为断言 worker task 已被收割（`worker_finished.is_set()`，对齐原 `thread.join` 语义）。
- 复跑（CI 对齐命令，同环境同用例集）：**2276 passed, 30 deselected, 0 failed, 219.06s**——基线 2274 + 新增 2，无新增失败。
- 残留：F2（stalled 漂移，已判语义等价）、F5（协议与实现结构不一致，A0/A1 遗留）维持 P3 non-blocking，建议 C 波次接 Codex/Pi 前处理 F5。

## 最终结论

**pass**。F1 修复正确且有回归测试锁定，F3/F4 一并处理，全量测试 2276 passed 与基线+新增预期一致；F2/F5 为非阻塞遗留，不构成本波次合入障碍。
