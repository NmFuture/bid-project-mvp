...
// 后端那张表由技术标和商务标共用（BusinessParseService 继承同一套回调），所以折算统一放在
// 前端这一共享展示层做，不动后端刻度。
...
// monotonicCounters=false 对应商务标历史行为：只做百分比/phasePercent 单调合并，
// 不回填 opencodeOutput 与 current/total 计数；默认 true 为技术标行为。
export const mergeMonotonicParseProgress = (previous = null, incoming = null, { monotonicCounters = true } = {}) => {
  ...
  if (!monotonicCounters) return merged
