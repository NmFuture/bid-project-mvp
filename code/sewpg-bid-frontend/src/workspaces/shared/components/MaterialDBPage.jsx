...
import {
  ...
  pickDefaultFolder,
  projectLabel,
  readStoredUploadKind,
  safeMessage,
  statusColor,
  tagInputPreview,
  toSizeLabel,
} from './materialDbUtils'

// 技术标/商务标原始素材库的共享页面：两轨 83% 相同的目录树、上传、标签、
// 切分、预览主流程全部收敛在这里；轨道差异（bidType、目录保护口径、素材层级
// 上传流、多选/自动打标签、项目联动、排序与重载策略、页头组件等）通过 config 注入。
// 行为口径与合并前 TechnicalMaterialDB / BusinessMaterialDB 保持一致。
...
function TreeNode({
  ...
  const canDragFolder = !isFolderDeleteProtected(normalizedNodePath)
  const canDeleteThisFolder = Boolean(normalizedNodePath) && !isFolderDeleteProtected(normalizedNodePath)
  const canRenameThisFolder = Boolean(normalizedNodePath) && !isFolderRenameProtected(normalizedNodePath)
            ...
            {directFiles.length > 0 && (
              <div className="mt-0.5 space-y-0.5">
                {directFiles.map((item) => {
                  ...
                  const canSplit = item.bidType === bidType && extOf(item.name) === 'docx'
                    ...
                    >
                      ...
                      <span className={`flex min-w-[8rem] shrink-0 items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100 ${bulkMode ? 'hidden' : ''}`}>
                        ...
                        {canSplit && (
                          <button
                            type="button"
                            title={`切分${trackName}素材`}
                            aria-label={`切分${trackName}素材 ${item.name || item.id}`}
                            onClick={(event) => {
                              event.stopPropagation()
                              onSplitFile?.(item)
                            }}
              ...
              <div className="mt-0.5">
                {node.children.map((child) => (
                  <TreeNode
                    ...
                    bidType={bidType}
                    trackName={trackName}
                    isFolderDeleteProtected={isFolderDeleteProtected}
                    isFolderRenameProtected={isFolderRenameProtected}
                  />
                ))}
              </div>
            )}
...
export default function MaterialDBPage({ config, showToast = () => {} }) {
  const {
  ...
  const uploadBidType = activeBidType
  const materialsBasePath = workspaceRoute(workspace, '/materials')
  const materialKindMeta = (value) =>
    materialKindOptions.find((item) => item.value === value) || materialKindOptions[2]
  const materialTierMeta = (value) =>
    (tierOptions || []).find((item) => item.value === value) || (tierOptions || [])[0] || {}
  const navigate = useNavigate()
  const [searchParams, setSearchParams] = useSearchParams()
  // 解析页「确认参与」跳转带过来的定位参数：folder 定位项目定制目录，projectId 用于回到项目工作区。（仅技术轨启用）
  const locateFolderPath = features.linkedProjectFlow ? normalizePath(searchParams.get('folder') || '') : ''
  const linkedProjectId = features.linkedProjectFlow ? String(searchParams.get('projectId') || '').trim() : ''
  ...
  const canDeleteFolder = Boolean(selectedFolderPath) && !isFolderDeleteProtected(selectedFolderPath)
  ...
  const loadLibrary = useCallback(async (options = {}) => {
    ...
    try {
      ...
      const normalizedTree = ensureMaterialRootNodes(
        normalizeTreeNodes(treeResponse?.tree || treeResponse?.items || treeResponse?.nodes || [], rootLabels),
        activeBidType,
      )
      const filteredTree = normalizedTree.filter((node) => normalizePath(node.path) === activeBidType)
      const visibleTree = features.sortLibrary ? sortNodesByName(filteredTree) : filteredTree
      setTree(visibleTree)
      ...
      // 商务轨保持原行为：选中目录变化由下方独立 effect 显式触发重载。
      ...
      const effectiveFolder = validPaths.has(currentSelected)
        ? currentSelected
        : pickDefaultFolder(visibleTree, activeBidType, rootPath)
      setCollapsedMap((prev) => {
        ...
        // 技术轨：仅首次加载时强制展开默认目录路径；后续静默刷新保留用户的展开/收起状态，不再覆盖。
        // 商务轨：保持原行为，每次重载都展开当前目录路径。
        return (isFirstLoad || features.expandOnEveryLoad) ? expandPathInMap(next, effectiveFolder) : next
      })
      ...
      setFilesPayload(
        payload
          ? (features.sortLibrary ? { ...payload, items: sortFilesByName(payload.items) } : payload)
  ...
  }, [activeBidType, features, filters.title, materialsAPI, rootLabels, rootPath, selectedFilterTags])
  ...
  // 商务轨保持原行为：选中目录变化会触发整库重载（技术轨刻意不触发，见 loadLibrary 注释）。
  const reloadOnFolderSelect = Boolean(features.reloadOnFolderSelect)
  const folderSelectFirstRunRef = useRef(true)
  useEffect(() => {
    if (!reloadOnFolderSelect) return undefined
    if (folderSelectFirstRunRef.current) {
      folderSelectFirstRunRef.current = false
      return undefined
    }
  ...
  }, [reloadOnFolderSelect, selectedFolderPath, loadLibrary])

  // 技术轨：标题筛选 400ms 防抖后写入 filters（商务轨原行为为逐键即改，见工具栏输入框绑定）。
  useEffect(() => {
    if (!features.searchDebounce) return undefined
    const timer = setTimeout(() => {
      setFilters((prev) => ({ ...prev, title: titleInput }))
    }, 400)
    return () => clearTimeout(timer)
  }, [features.searchDebounce, titleInput])
  ...
  }, [activeBidType, materialsAPI])

  useEffect(() => {
    if (!features.tierUpload || !showUploadModal) return undefined
    const timer = setTimeout(() => {
      loadUploadIdentityOptions()
    }, 0)
    return () => clearTimeout(timer)
  }, [features.tierUpload, showUploadModal, loadUploadIdentityOptions])

  useEffect(() => {
    if (!features.tierUpload || !showUploadModal || uploadCustomerId || !uploadCustomerName.trim()) return undefined
    ...
    return () => clearTimeout(timer)
  }, [features.tierUpload, customerOptions, showUploadModal, uploadCustomerId, uploadCustomerName])

  // 来源项目素材在后台复制，边复制边入库：轮询进度，跑完刷新一次目录树。（仅技术轨）
  useEffect(() => {
    if (!features.linkedProjectFlow || !linkedProjectId || !projectsAPI) return undefined
    let cancelled = false
    let timer = 0
    const poll = async () => {
      try {
        const detail = await projectsAPI.get(linkedProjectId)
        if (cancelled) return
  ...
  }, [features.linkedProjectFlow, linkedProjectId, loadLibrary, projectsAPI])
  ...
  const openUploadModal = (options = {}) => {
    ...
    // 商务轨：按落位目录推导素材层级；mode 保持原表达（rootTier ? 'tier' : 'tier'），默认即 'tier'
    const rootTier = features.tierUpload ? materialTierFromRootPath(targetPath, rootPaths, tierDirMap) : ''
  ...
  const performUpload = async (onConflict) => {
    ...
    try {
      const targetPath = features.tierUpload
        ? (uploadMode === 'path' ? uploadPath.trim() : '')
        : uploadPath.trim()
      ...
      // 技术轨：项目身份来自「确认参与」联动参数；商务轨：按素材层级从客户/项目选项推导
      const projectId = features.tierUpload
        ? (uploadMode === 'tier' && uploadMaterialTier === 'project' ? uploadProjectId.trim() : '')
        : linkedProjectId
      const projectCode = features.tierUpload && uploadMode === 'tier' && uploadMaterialTier === 'project'
        ? String(selectedProject?.projectCode || uploadProjectCode || projectId).trim()
        : ''
      const projectName = features.tierUpload && uploadMode === 'tier' && uploadMaterialTier === 'project'
        ? String(selectedProject?.name || uploadProjectName || '').trim()
        : ''
      const customerId = features.tierUpload
        ? (uploadMode === 'tier' && uploadMaterialTier === 'customer'
          ...
          : uploadMode === 'tier' && uploadMaterialTier === 'project'
            ...
            : '')
        : ''
      const customerName = features.tierUpload
        ? (uploadMode === 'tier' && uploadMaterialTier === 'customer'
          ...
          : uploadMode === 'tier' && uploadMaterialTier === 'project'
            ...
            : '')
        : ''
      const materialTier = features.tierUpload && uploadMode === 'tier' ? uploadMaterialTier : ''

      if (features.tierUpload ? (uploadMode === 'path' && !targetPath) : !targetPath) {
        setUploadError('请填写目标目录。')
        setUploading(false)
        return
      }
      if (!features.tierUpload && normalizePath(targetPath) !== activeBidType && !normalizePath(targetPath).startsWith(`${activeBidType}/`)) {
        setUploadError(`目标目录必须位于${activeBidType}素材库。`)
        setUploading(false)
        return
      }
      if (features.tierUpload && uploadMode === 'tier' && uploadMaterialTier === 'customer' && !customerId) {
        setUploadError('请选择客户。')
        setUploading(false)
        return
      }
      if (features.tierUpload && uploadMode === 'tier' && uploadMaterialTier === 'project' && !projectId) {
      ...
      payload.append('businessMaterialKind', uploadBidType === activeBidType ? 'other' : '')
      ...
      closeUploadModal()
      await loadLibrary({ silent: true })
      if (features.uploadAfterSplit) {
        const uploadedDocx = (result?.items || []).find((item) => item?.bidType === activeBidType && extOf(item?.name) === 'docx')
        if (uploadAfterSplit && uploadedDocx) {
          showToast(`上传完成，正在打开${trackName}素材切分审核。`)
          await openMaterialSplitModal(uploadedDocx)
        } else if (uploadAfterSplit) {
          showToast(`上传完成，但未找到可切分的${trackName} docx 文件。`, 'warning')
        }
      }
    } catch (e) {
  ...
  const handleDeleteFolder = async (path = selectedFolderPath) => {
    ...
    const canDeleteTarget = Boolean(targetPath) && !isFolderDeleteProtected(targetPath)
    if (!canDeleteTarget) {
  ...
  const handleRenameFolder = async (path = selectedFolderPath) => {
    const targetPath = normalizePath(path)
    if (!targetPath || isFolderRenameProtected(targetPath)) {
    ...
    try {
      const result = await materialsAPI.raw.renameFolder({ path: targetPath, newName: nextName.trim() })
  ...
  const updateMaterialUsageKind = async (item) => {
    if (!item?.id || item.bidType !== activeBidType) return
    const current = materialKindOptions.some((option) => option.value === item.businessMaterialKind)
    ...
    try {
      ...
      showToast(result?.message || `已标记为${materialKindMeta(nextKind).label}`)
      if (previewItem?.id === item.id) {
        setPreviewItem((prev) => ({
          ...(prev || {}),
          businessMaterialKind: nextKind,
          businessMaterialKindLabel: materialKindMeta(nextKind).label,
        }))
      }
      await loadLibrary({ silent: true })
    } catch (e) {
  ...
  const saveMaterialTags = async () => {
    ...
    setTagEditorError('')
    try {
      const result = await materialsAPI.raw.updateFile(tagEditorItem.id, {
  ...
  const handleAutoTags = async () => {
    ...
    try {
      const result = await materialsAPI.raw.autoTags({ targetPath: selectedFolderPath })
  ...
  const loadMaterialSplitPlan = async (item, { aiMode = 'auto' } = {}) => {
    const payload = await materialsAPI.raw[splitPreviewMethod](item.id, { targetPath: item.folderPath || selectedFolderPath, aiMode })
    setSplitPlan({
      ...payload,
      fragments: (payload?.fragments || []).map((fragment) => ({
        ...
        fileName: fragment.suggestedFileName || `${fragment.title || `${trackName}素材片段`}.docx`,
      })),
    })
  }

  const openMaterialSplitModal = async (item) => {
    if (!item?.id) return
    if (item.bidType !== activeBidType) {
      showToast(`素材切分入口当前仅支持${activeBidType}素材。`, 'error')
      return
    }
    ...
    setSplitPlan(null)
    try {
      await loadMaterialSplitPlan(item, { aiMode: 'auto' })
    } catch (e) {
  ...
  const rerunMaterialSplitWithAi = async () => {
    if (!splitSourceItem?.id) return
    setSplitLoading(true)
    setSplitError('')
    try {
      await loadMaterialSplitPlan(splitSourceItem, { aiMode: 'force' })
  ...
  const confirmMaterialSplit = async () => {
    ...
    setSplitError('')
    try {
      const result = await materialsAPI.raw[splitConfirmMethod](splitSourceItem.id, {
  ...
  const handlePreviewCleaned = async (item) => {
    ...
    try {
      const payload = canPreviewCleaned(item)
        ? await materialsAPI.raw.previewCleanedFile(item.id)
        : await materialsAPI.raw.previewOriginalFile(item.id)
  ...
  const handleDownloadFile = (item) => {
    if (!item?.id) return
    window.open(materialsAPI.raw.contentUrl(item.id), '_blank', 'noopener')
  }
  ...
  const handleMoveDrop = async (event, targetPath) => {
    ...
    if (isFolderDeleteProtected(sourcePath)) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const result = await materialsAPI.raw.batchDelete({ fileIds: ids })
      const failedCount = (result?.failed || []).length
  ...
  const saveBulkTags = async () => {
    ...
    setBulkOperating(true)
    try {
      const result = await materialsAPI.raw.batchTags({
        fileIds: ids,
        tags: nextTags,
      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="grid w-full min-w-0 grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] xl:w-1/2 xl:max-w-[36rem] xl:flex-none">
          <label>
            ...
            <input
              value={features.searchDebounce ? titleInput : filters.title}
              onChange={(e) => (features.searchDebounce ? setTitleInput(e.target.value) : updateFilter('title', e.target.value))}
              placeholder="文件标题"
        ...
        <div className="flex flex-wrap items-center justify-start gap-1.5 xl:justify-end" role="toolbar" aria-label="素材目录工具栏">
          ...
          <button
            type="button"
            onClick={() => openUploadModal(features.tierUpload ? { mode: 'path', targetPath: selectedFolderPath } : { targetPath: selectedFolderPath })}
    ...
    <>
      ...
      {previewLoading ? (
        ...
        extOf(previewItem?.name) === 'pdf' ? (
          <iframe
            src={materialsAPI.raw.previewContentUrl(previewItem.id)}
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3">
      ...
      {linkedProjectId && (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-surface-container-high p-4">
          ...
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={() => navigate(projectRoute(linkedProjectId, '/template-directory', workspace))}
              ...
              {displayTree.length > 0 ? (
                displayTree.map((node) => (
                  <TreeNode
                    ...
                    onSplitFile={openMaterialSplitModal}
              ...
              {features.tierUpload ? (
                ...
                <label className="block text-sm text-on-surface-variant">
                  ...
                  <input
                    ...
                    placeholder={activeBidType}
                  />
                </label>
              )}

              {uploadBidType === activeBidType && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {features.uploadAfterSplit && (
                  <label className="rounded-lg bg-primary/5 px-3 py-2 text-sm text-on-surface-variant">
                    ...
                    <span className="mt-1 block text-xs text-outline">
                      仅对{activeBidType} docx 生效；原文件保留，审核后生成子素材。
                    </span>
                  </label>
                )}
              </div>
      ...
      {splitModalOpen && (
        <div className="dialog-overlay fixed inset-0 z-50 bg-black/40 flex items-start justify-center overflow-hidden p-3 sm:p-4">
          <div role="dialog" aria-modal="true" aria-label={`${trackName}素材切分审核`} className="wizard-modal-surface flex h-[calc(100dvh-1.5rem)] max-h-[60rem] w-full max-w-5xl flex-col overflow-hidden rounded-lg border border-surface-container-high bg-surface-container-lowest animate-float-in sm:h-[calc(100dvh-2rem)]">
            <div className="px-5 sm:px-6 py-4 border-b border-surface-container-high flex items-center justify-between shrink-0">
              <div className="min-w-0">
                <h2 className="text-lg font-headline font-bold text-on-surface">{trackName}素材切分审核</h2>
                <p className="mt-1 truncate text-xs text-outline">
                  母文件：{splitSourceItem?.name || '-'}；确认后将生成子素材并进入{activeBidType}原始素材库。
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={rerunMaterialSplitWithAi}
                ...
                <button onClick={closeMaterialSplitModal} className="close-plain text-on-surface-variant hover:text-primary transition-colors" aria-label="关闭">
                  <span className="material-symbols-outlined">close</span>
                </button>
              </div>
            </div>
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <label className="flex min-w-0 flex-1 items-start gap-2">
                              ...
                              <span className="min-w-0">
                                ...
                                <span className="mt-1 block text-xs text-outline">
                                  {fragment.materialType || `${trackName}素材片段`} · 置信度 {Math.round(Number(fragment.confidence || 0) * 100)}%
                                </span>
                              </span>
                            </label>
            ...
            <div className="px-5 sm:px-6 py-4 border-t border-surface-container-high flex flex-wrap items-center justify-between gap-3 bg-surface-container-low rounded-b-xl shrink-0">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={closeMaterialSplitModal} className="px-4 py-2 text-sm text-on-surface-variant hover:bg-surface-container-high rounded-lg">
                  取消
                </button>
                <button
                  onClick={confirmMaterialSplit}
