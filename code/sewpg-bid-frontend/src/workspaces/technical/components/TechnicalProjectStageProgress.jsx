...
const technicalStageCache = new Map()
...
export default function TechnicalProjectStageProgress({
  ...
  const cachedStages = projectId ? technicalStageCache.get(projectId) || [] : []
  ...
  const loadStages = useCallback(async () => {
    ...
    const cached = technicalStageCache.get(projectId) || []
    if (cached.length) {
      setStages(cached)
      setLoading(false)
    } else {
    ...
    try {
      ...
      technicalStageCache.set(projectId, nextStages)
