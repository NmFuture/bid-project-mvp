...
const SELECT_CLASS =
  'h-8 w-full rounded border border-outline-variant/70 bg-surface-container-lowest px-1.5 text-xs text-on-surface focus:border-primary focus:outline-none'
...
export default function TechnicalBrandPicksModal({ projectId, onClose, showToast }) {
  ...
  const updatePick = (component, materialName) => {
    const current = pickByComponent.get(component) || { component, brand: '', reason: '' }
    const next = componentNames.map((name) => {
      const pick = name === component ? { ...current, materialName } : pickByComponent.get(name)
      return pick || { component: name, brand: '', materialName: '', reason: '' }
    })
      ...
      await technicalGapsAPI.saveBrandPicks(projectId, {
        picks: componentNames.map((name) => {
          ...
          return {
            ...
            materialName: String(pick.materialName || ''),
  ...
  return (
    <Dialog open onClose={busy ? undefined : onClose} size="lg">
                    ...
                    <tr key={component} className="odd:bg-surface-container-lowest even:bg-surface-container-low/40">
                      ...
                      <td className="border-b border-outline-variant/40 px-2 py-2 align-top">
                        <select
                          ...
                          onChange={(event) => updatePick(component, event.target.value)}
                        >
                          <option value="">（空着，交人工）</option>
                          {candidates.map((name) => (
      ...
      <DialogFooter>
        <Button type="button" variant="quiet" size="sm" onClick={handleRegenerate} disabled={Boolean(busy)}>
          {busy === 'regenerate' ? '重新选取中...' : '重新问一次 AI'}
        </Button>
        <div className="flex-1" />
        <Button type="button" variant="quiet" size="sm" onClick={onClose} disabled={Boolean(busy)}>
          取消
        </Button>
        <Button type="button" variant="primary" size="sm" onClick={handleSave} disabled={Boolean(busy) || !componentNames.length}>
          {busy === 'save' ? '保存中...' : '保存'}
        </Button>
      </DialogFooter>
    </Dialog>
