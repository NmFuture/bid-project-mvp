...
export default function TechnicalOnlyOfficeWorkspace({
  sidebar,
  documentTitle,
  documentSubtitle,
  documentMeta,
  ...
  return (
    <div className={['relative', className].join(' ')}>
      ...
      >
        ...
        >
          <div className="flex min-h-[56px] flex-wrap items-center justify-between gap-2 border-b border-surface-container-high bg-surface-container-low px-4 py-2">
            <div className="min-w-0">
              <h3 className="truncate text-base font-semibold text-on-surface">
                {documentTitle}
              </h3>
              {documentSubtitle ? (
                <p className="mt-0.5 truncate text-xs text-outline" title={typeof documentSubtitle === 'string' ? documentSubtitle : undefined}>
                  {documentSubtitle}
                </p>
              ) : null}
            </div>
