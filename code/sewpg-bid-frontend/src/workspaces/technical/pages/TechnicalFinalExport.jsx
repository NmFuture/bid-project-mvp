...
export default function TechnicalFinalExport({ showToast }) {
  const { id } = useParams()
  ...
  const loadData = useCallback(async () => {
    ...
    setError('')
    try {
      const payload = await technicalDocumentAPI.final(id)
      setData(payload)
    } catch (e) {
  ...
  return (
    <div className="stage-page technical-ui-shell flex flex-col gap-6 animate-fade-in w-full max-w-none">
      ...
      <DataCard className="!p-0 overflow-hidden technical-panel min-h-[360px]">
        <div className="technical-section-head flex items-center justify-between gap-3 border-b border-surface-container-high px-4 py-3">
        ...
        <div className="grid gap-4 bg-surface-container-low p-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(320px,0.5fr)]">
          ...
          <div className="technical-panel flex min-h-[220px] flex-col justify-between rounded-md border border-surface-container-high bg-surface-container-lowest p-4">
            <div>
