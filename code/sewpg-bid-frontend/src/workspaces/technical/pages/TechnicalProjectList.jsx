  ...
  return (
    <div className="project-list-page flex min-h-0 w-full flex-col gap-4">
      <PageHeader
        ...
        description={`当前工作区只展示${lockedBidType}项目，素材库、Wiki 和日志也按同一标类隔离。`}
        actions={(
          <Button
            type="button"
      ...
      {!projects.length ? (
        ...
        <div className="mt-2 flex min-h-0 flex-1 flex-col gap-3">
          <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,22rem),1fr))] gap-3 xl:hidden" aria-label="项目列表">
          ...
          <div className="project-table-frame hidden min-h-0 flex-1 flex-col border border-outline-variant/50 bg-surface-container-lowest xl:flex">
            <div className="min-h-0 flex-1 overflow-x-auto overflow-y-auto" tabIndex={0} aria-label="项目数据表，可水平滚动">
                ...
                {projects.map((project) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-4 text-sm text-on-surface-variant">{project.bidType || '-'}</td>
