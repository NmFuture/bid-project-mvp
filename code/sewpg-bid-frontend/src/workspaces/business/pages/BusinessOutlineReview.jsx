...
import { businessGapsAPI, businessOutlineAPI, businessStagesAPI } from '../../../api'
...
export default function BusinessOutlineReview({ showToast }) {
  const { id } = useParams()
  ...
  const loadData = useCallback(async () => {
    ...
    setError('')
    try {
      const payload = await businessOutlineAPI.get(id)
  ...
  const focusSourceRef = useCallback(async (basisRef) => {
    ...
    if (basisRef?.type === 'tender' && refFileId && refFileId !== activeFileId) {
      try {
        const payload = await businessOutlineAPI.get(id, { fileId: refFileId })
        setTenderPreview(payload?.tenderPreview || null)
        setOnlyofficeError('')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = await businessOutlineAPI.save(id, { nodes: nodesToSave })
      const nextNodes = Array.isArray(payload?.nodes) ? payload.nodes : []
      setNodes(nextNodes)
      setDirty(false)
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (dirty) {
        const nodesToSave = renumberOutlineNodes(nodes)
        const saved = await businessOutlineAPI.save(id, { nodes: nodesToSave })
