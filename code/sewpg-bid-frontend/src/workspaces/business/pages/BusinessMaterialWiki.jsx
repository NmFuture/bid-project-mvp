...
export default function BusinessMaterialWiki({ showToast = () => {} }) {
  ...
  const handleCreateNode = async (isFolder = false) => {
    ...
    try {
      const payload = await businessMaterialsAPI.wiki.create({
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const payload = await businessMaterialsAPI.wiki.update(selectedNodeId, {
        title,
  ...
  const handleDeleteNode = async (nodeId = selectedNodeId, title = selectedNode?.title || '') => {
    ...
    try {
      const payload = await businessMaterialsAPI.wiki.delete(nodeId, { bidType: activeBidType })
      applyPayload(payload)
  ...
  const handleRefreshSummary = async () => {
    ...
    try {
      const payload = await businessMaterialsAPI.wiki.refreshSummary(selectedNodeId, { bidType: activeBidType })
      applyPayload(payload)
      showToast('摘要已刷新')
    } catch (e) {
      console.error(e)
  ...
  const handleUploadAttachment = async (file) => {
    ...
    try {
      ...
      const payload = await businessMaterialsAPI.wiki.uploadAttachment(selectedNodeId, formData)
      applyPayload(payload)
      showToast('附件上传成功')
    } catch (e) {
      console.error(e)
  ...
  const handleDeleteAttachment = async (attachment) => {
    ...
    try {
      const payload = await businessMaterialsAPI.wiki.deleteAttachment(attachment.id, { bidType: activeBidType })
      applyPayload(payload)
  ...
  const handleMoveNode = async (movingId, targetNode, mode) => {
    ...
    setMovingNode(true)
    try {
      const payload = await businessMaterialsAPI.wiki.move(movingId, {
