...
const compactProjectStages = (rawStages = []) => {
  ...
  return BUSINESS_COMPACT_STAGE_GROUPS.map((group) => {
...
const businessStageRoute = (projectId, stage) => {
  const routeStageId = toStageId(stage?.routeStageId || stage?.id)
  const routePath = BUSINESS_STAGE_ROUTES[routeStageId]
  ...
  const handleStageClick = useCallback(
    (stage) => {
      ...
      const route = stage.routePath
        ? projectRoute(projectId, stage.routePath, BUSINESS_WORKSPACE)
        : businessStageRoute(projectId, stage)
      if (!route) {
