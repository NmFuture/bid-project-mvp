...
export default function BusinessMaterialsViewSwitch({
  ...
  const normalizedBasePath = String(basePath || '/workspace/business/materials').replace(/\/+$/, '')

  return (
    <div className="overflow-hidden rounded-lg border border-outline-variant/45 bg-surface-container-lowest">
      <div className="flex flex-col gap-3 px-4 py-3 lg:px-5 xl:min-h-[58px] xl:flex-row xl:items-center xl:justify-between">
        <div className="flex min-w-0 flex-1 flex-col gap-3 lg:flex-row lg:items-center">
          <div className="min-w-0 lg:w-[220px] xl:w-[250px]">
            <div className="flex min-w-0 items-center gap-2">
              <span aria-hidden="true" className="h-4 w-1 shrink-0 rounded-full bg-primary/80" />
              <h1 className="truncate text-base font-headline font-bold text-on-surface">{title || '商务标素材库'}</h1>
            </div>
            {subtitle ? (
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <span className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/10 px-3 text-xs font-semibold text-primary">
              <span aria-hidden="true" className="material-symbols-outlined text-[16px]">request_quote</span>
              商务标
            </span>
