...
const priorityStyleMap = {
  high: 'bg-error-container/25 text-error',
  medium: 'bg-tertiary-fixed/25 text-tertiary',
...
export default function GapRecognition({ showToast }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-6xl mx-auto w-full">
      ...
      <PageHeader
        title="S4 素材缺口识别"
        description="先触发识别，系统将返回缺失素材清单与补充优先级；确认后进入 S5 进行补交。"
        leftExtra={(
          <button
        ...
        {!isCompleted ? (
          <div className="h-[340px] px-6 py-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-on-surface-variant max-w-xl leading-relaxed">
              点击“触发识别”后，系统会根据 S1/S2 结果识别缺失素材，并给出每项补充优先级。
            </p>
            <button
              onClick={handleRunDetection}
              disabled={running}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-surface-container-low border-b border-surface-container-high">
                  ...
                  <th className="px-6 py-3 text-left font-semibold text-on-surface">优先级</th>
                ...
                {missingItems.map((item) => (
                  <tr key={item.id} className="border-b border-surface-container-high hover:bg-surface-container-low/60">
                    ...
                    <td className="px-6 py-3">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${priorityStyleMap[item.priority] || priorityStyleMap.low}`}>
                        {priorityLabelMap[item.priority] || '低'}优先级
                      </span>
                    </td>
