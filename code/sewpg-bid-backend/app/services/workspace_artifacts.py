...
def _profile_for_workspace(profile: Any = "技术标") -> ParseProfile:
    if isinstance(profile, ParseProfile):
        return profile
    return resolve_parse_profile(str(profile or "技术标"))


def workspace_dir(project_id: str, profile: Any = "技术标") -> Path:
    return settings.documents_dir / project_id / _profile_for_workspace(profile).workspace_dirname
...
def workspace_appendices_dir(project_id: str, profile: Any = "技术标") -> Path:
...
def workspace_parse_dir(project_id: str, profile: Any = "技术标") -> Path:
...
def workspace_stage_dir(project_id: str, stage_name: str, profile: Any = "技术标") -> Path:
    return workspace_dir(project_id, profile) / stage_name
...
def promote_parse_artifacts_to_workspace(
    ...
    *,
    ...
    """Copy the current S1 parse JSON and generated Word files into the project workspace."""
