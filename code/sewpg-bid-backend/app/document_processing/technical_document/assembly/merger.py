...
# 单一事实源：本文件是技术标成稿正文合并（assembly_plan → 母版 docx）逻辑的唯一事实源。
# opencode/skills/bid-tech-assembler/scripts/merger.py 是它在 opencode
# 容器环境的 vendored 拷贝（仅同包模块的导入适配段不同）。修改本文件的合并逻辑后
