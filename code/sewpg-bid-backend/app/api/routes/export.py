...
async def export_check(project_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "suggestedFileName": f"投标文件_{project_id}_{__import__('datetime').datetime.now().strftime('%Y%m%d')}",
    }


@router.post("/api/projects/{project_id}/export", response_model=None)
async def export_document(project_id: str, data: dict[str, Any] = Body(default_factory=dict)) -> Any:
    store.get_project(project_id)
    coverage = store.get_coverage(project_id)
    ...
    return {
        "message": "Exported",
        "fileUrl": f"/downloads/{file_name}.{fmt}",
