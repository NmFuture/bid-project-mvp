...
def ensure_python_deps() -> None:
    if convert_from_path is None or pdfinfo_from_path is None:
        raise RuntimeError(
            "Missing required Python package: pdf2image. "
            "Install it with `pip3 install --user pdf2image python-docx` and retry."
        )
...
def calc_dpi_via_pdf(input_path: str, max_w_px: int, max_h_px: int) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="soffice_profile_") as user_profile:
        with tempfile.TemporaryDirectory(prefix="soffice_convert_") as convert_tmp_dir:
            ...
            match = re.search(r"(\d+)\s*x\s*(\d+)\s*pts", size_val)
            if not match:
                raise RuntimeError("Unrecognized PDF page size format.")
            width_pts = int(match.group(1))
            height_pts = int(match.group(2))
...
def rasterize(doc_path: str, out_dir: str, dpi: int) -> Sequence[str]:
    """Rasterize DOCX (or similar) to images placed in out_dir and return their paths."""
...
def main() -> None:
    ...
    parser.add_argument("input_path", type=str, help="Path to the input DOCX file (or compatible).")
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1600,
        help="Approximate maximum width in pixels after isotropic scaling (default 1600).",
    )
    parser.add_argument(
        "--height",
        type=int,
        default=2000,
        help="Approximate maximum height in pixels after isotropic scaling (default 2000).",
    )
    parser.add_argument(
        "--dpi",
        type=int,
