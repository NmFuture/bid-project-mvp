...
def _index_document_nav(document_meta: dict[str, Any], nav_path: Path) -> IndexedDocument:
    ...
    def add_table(
        ...
        *,
        ...
        page_no = int(raw_table.get("pageNo") or (source_block or {}).get("pageNo") or 0)
        evidence_id = str(
            raw_table.get("evidenceId")
            or (source_block or {}).get("evidenceId")
            or table_id
        )
        table = {
            "id": table_id,
    ...
    for raw_block in payload.get("blocks") if isinstance(payload.get("blocks"), list) else []:
        ...
        evidence_id = str(
            raw_block.get("evidenceId")
            or raw_block.get("id")
            or f"{document_id}:B{body_index:06d}"
        )
        if level:
            heading_stack = _update_heading_stack(heading_stack, level, text)
            headings.append({"id": evidence_id, "level": level, "title": text, "bodyIndex": body_index})
        blocks.append(
            {
                "id": evidence_id,
                "evidenceId": evidence_id,
                "documentId": document_id,
                "bodyIndex": body_index,
                "pageNo": int(raw_block.get("pageNo") or 0),
                "type": block_type,
                "text": text,
                "headingLevel": level,
                "headingPath": _heading_path(heading_stack),
