...
- 判据自包含（`own_table_limit`，runner:670）：blankDocx 内第二个**编号不同的**附表标题（或"附件"标题）之后的表格，全部视为越界内容，不作为填写目标。
- 同编号重复标题是**真续表**，不算边界；边界之内的多张表全部纳入字段识别与填写（`AppendixSpec.own_tables`）。
- 同一判据也用于金标评分（`eval_fill_vs_golden.py` v3 起），保证填写与评分口径一致。

## 2. C.1 / C.2 / C.3 增强词典与专题守门

附表 C.1（总体技术参数）、C.2/C.3（部件参数）维护增强概念词典（`C1_CONCEPTS` / `C2_C3_CONCEPTS`，runner:59/103）：每个概念挂多个同义标签（如 `额定功率` = 单机容量/机组额定功率/单机功率）。LLM 判断是附表填写的唯一模式，取值由 agent 完成；词典与打分（`score`，runner:2570）在 `--apply` 阶段用于**冲突降级与值守门**——agent 取值与高分事实明显冲突时降级 manual，不再作为取值依据。

伴随的专题守门规则（均来自金标反评教训，改动前先跑评估）：

- 数值尾部星号/脚注不是值的一部分（如 `10.7*` → `10.7`，`normalize_value_for_field`）。
- 扫风面积可按 π(D/2)² 纯几何派生；但**总扫风面积与单位千瓦扫风面积量纲差四个数量级，不得互相顶替**（`score`）。
...
所有附表走同一条链路：`--prepare` 做主题识别（`detect_appendix_spec`）与字段/清单抽取（`extract_target_fields`），产出 fill_brief.json；agent 读简报与素材逐格取值，写 fill_plan.json；`--apply` 逐格校验（`collect_plan_fills`：坐标合法性、证据摘录溯源、值可用性）并按事实冲突降级（`plan_value_conflict`），最后保格式写回原 Word（`fill_doc`）并产出报告（`finalize_fill_result`）。
...
- runner 只接受单附表 manifest（`blankSource`/`appendixTask`）；批量填写由后端 body-fill 批次逐条调度，每条独立 manifest、独立产物。
- 解析出的"副表"只有标题页、没有表格时：报告记 `targetFieldCount=0`，不中断任务。
