...
- 脚本负责读取 Word/Excel、识别待填单元格、自动选材、抽取候选事实、字段映射、写入原 Word 和生成报告。
- 关键规则速记：S1 越界表剔除（第二个编号不同的附表标题之后不填）；C.1/C.2/C.3 走增强词典，其他附表走通用链路；支持单目标或批量；只有标题页的"副表"原样复制不中断批量。
- 以上规则的完整细则、判据函数定位和金标评估方法见 `references/rules.md`；修改填写行为前必读，改动词典/评分规则前后各跑一次 `scripts/eval_fill_vs_golden.py` 对比得分。
