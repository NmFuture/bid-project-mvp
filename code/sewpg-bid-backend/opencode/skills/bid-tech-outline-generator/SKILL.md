...
**稀疏章按连续通读构建。** 当 `decision-next` 返回 `authoring_mode=sparse_chapter`（本章模板二级节点稀疏），本章需要从招标原文构建子目录：先通读完整招标目录，自主圈定与本章主题对应的少数上级章节；用 `section --max-chars 30000` 连续通读这些章节原文，不逐段跳读，也不以 `search` 发散探索（全会话最多 2 次，仅用于跨章定位）；从已读原文自主提炼响应单元，标题与粒度由你决定，一次性提交全部新增。证据规则不变：每个新增仍须 `reason + evidence_id`。
...
当后端明确标记为“附表并行预判会话”时，改用 `appendix-predecision-next` 和 `appendix-predecision-batch`。此模式只提交 `appendix_id + decision + reason`，不提交 `node_id`、`parent_id` 或 `root_addition`；正文合并后由后端生成节点并执行正式 composition 校验。
