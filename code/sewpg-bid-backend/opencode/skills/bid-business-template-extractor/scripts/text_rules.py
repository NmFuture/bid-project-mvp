...
FORMAT_REGION_TOKENS = BID_FORMAT_REGION_TOKENS

MAJOR_SECTION_RE = re.compile(r"^第[一二三四五六七八九十百千0-9]+章\s*")
CATALOG_TITLE_RE = re.compile(r"^目\s*录$|^目\s*次$")
...
CATALOG_ITEM_RE = re.compile(
    r"^(?:(?:特殊)?附件\s*[0-9一二三四五六七八九十]+[A-Z]?|[0-9]+(?:\.[0-9]+)*[、.．]?|[一二三四五六七八九十]+[、.．])\s*\S+",
    re.IGNORECASE,
)
...
def is_catalog_heading(text: str) -> bool:
    return bool(CATALOG_TITLE_RE.match(clean_text(text).replace(" ", "")))


def looks_like_catalog_item(text: str) -> bool:
    normalized = clean_text(text)
    compact = compact_text(normalized)
    if not compact or len(compact) > 60:
        return False
    if looks_like_body_sentence(normalized) or looks_like_list_item_or_field(normalized):
        return False
    return bool(CATALOG_ITEM_RE.match(normalized))
...
def is_format_region_heading(text: str) -> bool:
    ...
    if not normalized:
        return False
    if any(token in normalized for token in EXCLUDED_FORMAT_REGION_TOKENS):
        return False
    return any(token in normalized for token in BID_FORMAT_REGION_TOKENS) and len(normalized) <= 60


def is_excluded_format_region_heading(text: str) -> bool:
    normalized = compact_text(text)
    if not normalized:
        return False
    return any(token in normalized for token in EXCLUDED_FORMAT_REGION_TOKENS) and len(normalized) <= 60
...
def has_table_template_topic(text: str) -> bool:
    normalized = compact_text(text)
    return any(token in normalized for token in TABLE_TEMPLATE_TITLE_TOKENS)
...
def has_short_heading_shape(text: str) -> bool:
    ...
    if not compact or len(compact) > 40:
        return False
    if re.match(r"^[\u4e00-\u9fffA-Za-z（）()]{1,12}[：:]\s*\S+", normalized):
        return False
    if compact.endswith(("正文", "内容", "说明")):
        return False
...
def looks_like_list_item_or_field(text: str) -> bool:
    ...
    if not normalized:
        return False
    if has_numbering_prefix(normalized):
        return False
...
def looks_like_body_sentence(text: str) -> bool:
    ...
    return any(mark in normalized for mark in ("。", "；", ";")) and not (
        APPENDIX_TITLE_RE.match(normalized) or FORMAT_CODE_RE.search(compact) or has_numbering_prefix(normalized)
    )
...
def title_strength(text: str, block: dict) -> tuple[int, list[str]]:
    ...
    style_name = str(block.get("styleName") or "")
    if "标题" in style_name or style_name.lower().startswith("heading"):
        score += 18
        if "heading_style" not in signals:
            signals.append("heading_style")
    ...
    if has_table_template_topic(compact):
        score += 15
        signals.append("table_template_topic")
    if len(compact) <= 36:
        score += 8
