...
- **非 docx 素材按需转 Word**。后端取 raw 原件，Excel 逐格转表格、PDF 逐页转图片，转完再嵌；不用素材恰好带的自动转换清洗稿（那份会丢合并单元格、认错表头）。
- **插入范围是闭区间 `[起点节, 终点节]`**。占位符写 `[文件-起点-终点，待插入]` 取「起点节 + 中间所有章 + 终点节」，写 `[文件-某节，待插入]` 取单节（起点=终点），写 `[文件-完整插入，待插入]` 取整份（`完整插入` 是保留字）。「节」= 标题行本身到下一个同级或更高级标题之前，含子标题、表格、图。
- **锚点精确优先**：剥掉编号前缀后全等且唯一才用，精确无命中再退子串，多命中不猜。`发电量结果` 会被 `方案及发电量结果` 子串命中，直接用子串会插错整个上级章节。
- **文件名边界由素材索引决定**：占位符按 `-` 从最长前缀往短试，第一个在素材库命中的前缀就是文件名——素材名本身带连字符（型式认证素材有四个），按语法固定拆会切碎。一个前缀都不命中时报「素材未找到」，不报格式错。
