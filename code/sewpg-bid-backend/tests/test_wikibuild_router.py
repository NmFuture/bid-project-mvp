...
def load_router_module():
    module_name = "wikibuild_router_test"
...
class WikibuildRouterTests(unittest.TestCase):
    def write_manifest(self, root: Path, payload: dict[str, object]) -> Path:
        manifest_path = root / "manifest.json"
        manifest_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        return manifest_path

    def test_resolve_runner_routes_business_manifest_to_business_skill(self) -> None:
        ...
        self.assertIn("bid-business-wiki-material-builder", str(runner))
    ...
    def test_resolve_runner_defaults_unknown_bid_type_to_technical_skill(self) -> None:
        ...
        self.assertEqual(runner.name, "run_from_manifest.py")
        self.assertIn("bid-tech-wiki-material-builder", str(runner))
    ...
    def test_main_dispatches_manifest_to_resolved_runner(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(sys, "argv", ["wikibuild_router.py", str(manifest_path)]),
            ):
                ROUTER.main()
