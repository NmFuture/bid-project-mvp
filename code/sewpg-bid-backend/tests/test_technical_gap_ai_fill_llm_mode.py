...
1. 始终走 LLM 模式：prompt 走 prepare→plan→apply 流程，不传 early_tool_command
   （等会话自然结束），OpencodeEngine 用 S4_LLM_FILL_TIMEOUT_SEC（缺省沿用
...
class _FakeOpencodeEngine:
    """记录构造参数与调用参数的 OpencodeEngine 替身。"""

    instances: list["_FakeOpencodeEngine"] = []
    ...
    async def run_bid_tech_table_filler_with_trace(self, prompt, **kwargs):
        ...
        return dict(_FakeOpencodeEngine.result)
...
class TableFillerLlmOnlyTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        _FakeOpencodeEngine.instances = []
        _FakeOpencodeEngine.error = None
        _FakeOpencodeEngine.result = {"outputFile": "unused"}
    ...
    def _run(self, *, timeout_sec=None) -> dict:
        with (
            ...
            patch.object(ai_fill, "OpencodeEngine", _FakeOpencodeEngine),
        ):
    ...
    def test_llm_prompt_and_no_early_tool_command(self) -> None:
        ...
        _FakeOpencodeEngine.result = {"outputFile": str(output)}
        result = self._run(timeout_sec=3600.0)
        self.assertEqual(result["outputFile"], str(output))
        client = _FakeOpencodeEngine.instances[0]
        # LLM 会话明显变长：独立超时生效
    ...
    def test_session_failure_raises_with_reason(self) -> None:
        _FakeOpencodeEngine.error = RuntimeError("connection refused")
    ...
    def test_missing_output_file_raises_with_reason(self) -> None:
        _FakeOpencodeEngine.result = {"outputFile": str(self.base / "not_written.docx")}
        with self.assertRaisesRegex(RuntimeError, "未产出有效输出文件"):
            self._run()

    def test_empty_output_file_raises_with_reason(self) -> None:
        _FakeOpencodeEngine.result = {"outputFile": ""}
        with self.assertRaisesRegex(RuntimeError, "未产出有效输出文件"):
            self._run()
