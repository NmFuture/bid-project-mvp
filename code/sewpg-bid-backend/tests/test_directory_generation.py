...
class DirectoryGenerationTests(unittest.TestCase):
    ...
    def test_generate_outline_for_project_runs_local_rules_without_opencode(self) -> None:
        ...
        with patch(
            ...
            side_effect=AssertionError("S2 directory generation must not call OpenCode"),
        ):
        ...
        self.assertEqual(payload["opencodeOutput"]["status"], "not_used")
        self.assertEqual(payload["opencodeOutput"]["engine"], "local-rule-engine")
    ...
    def test_generate_outline_records_generic_evidence(self) -> None:
        ...
        self.assertEqual(evidence["engine"], "local-rule-engine")
        ...
        self.assertFalse(any("华能" in __import__("json").dumps(item, ensure_ascii=False) for item in evidence["rules"]))
    ...
    def test_tender_candidate_zero_limit_means_unlimited_and_strips_page_numbers(self) -> None:
        ...
        paragraphs.extend((f"{index}. 前置章节{index} {100 + index}", None) for index in range(1, 30))
        paragraphs.append(("附表D.7 性能及考核承诺保证表 188", None))
        self._write_docx(tender_path, paragraphs)

        original_limit = settings.s2_toc_max_tender_candidates
        settings.s2_toc_max_tender_candidates = 0
        try:
            payload = generate_outline_for_project(project_id, {"outlineStrategy": "strict"})
        finally:
            settings.s2_toc_max_tender_candidates = original_limit
    ...
    def test_only_explicit_appendix_tables_are_auto_added_after_template_outline(self) -> None:
        ...
        self.assertIn("附表E.2 风电场保证电量计算折减因素及相关要求表", titles)
        ...
        appendix_index = titles.index("附表E.2 风电场保证电量计算折减因素及相关要求表")
    ...
    def test_added_items_do_not_shift_later_template_source_refs(self) -> None:
        from app.services.toc_engine import (
            OutlineEntry,
            TenderCandidate,
            TocRules,
            build_toc_items,
        ...
        template_outline = [
            OutlineEntry("1", "风资源评估与机位排布方案", 1, "template.docx", 1, "第1章 风资源评估与机位排布方案"),
            OutlineEntry("1.1", "项目风资源评估与机组选型排布及发电量计算", 2, "template.docx", 2, "1.1 项目风资源评估与机组选型排布及发电量计算"),
            OutlineEntry("2", "产品交付、考核及验收", 1, "template.docx", 3, "第2章 产品交付、考核及验收"),
            OutlineEntry("2.1", "设备安装、调试与试运行", 2, "template.docx", 4, "2.1 设备安装、调试与试运行"),
        ]
        tender_candidates = [
            TenderCandidate(
                "附表E.2 风电场保证电量计算折减因素及相关要求表",
                "附表E.2 风电场保证电量计算折减因素及相关要求表 194",
                "tender.docx",
                10,
                kind="appendix",
                number="附表E.2",
        ...
        items, _ = build_toc_items(
            template_outline=template_outline,
            tender_candidates=tender_candidates,
            attach_outline=[],
            rules=TocRules(
                max_level=9,
                max_tender_candidates=0,
                max_title_chars=120,
                auto_append_tender_requirements=False,
            ),
        )
    ...
    def test_get_directory_state_loads_rule_evidence_from_existing_file(self) -> None:
        ...
        self.assertEqual(state["ruleEvidence"]["engine"], "local-rule-engine")
    ...
    def test_rule_engine_progress_updates_before_completion(self) -> None:
        ...
        _handle_directory_progress(
            ...
            "generating_outline",
            {
                "templateHeadingCount": 4,
                "tenderCandidateCount": 2,
            },
        )

        state = store.get_directory_state(project_id)
        self.assertEqual(state["status"], "running")
        self.assertEqual(state["percentage"], 70)
        self.assertEqual(state["opencodeOutput"]["status"], "not_used")
        self.assertEqual(state["events"][-1]["step"], "rule_generation")
        self.assertIn("规则引擎", state["events"][-1]["message"])
    ...
    def test_background_job_updates_running_state_then_completes(self) -> None:
        ...
        self.assertEqual(completed_state["opencodeOutput"]["status"], "not_used")
        self.assertEqual(completed_state["opencodeOutput"]["parts"], [])
