...
- 涉及附表填写、待填写 Word、85% 人工基准验收时，同时先读 `doc/16-目标一与目标二最终目标.md`。
...
> 2026-05-03 项目信息口径：参与投标进入工作区后，`客户来源` 使用 `重要客户 / 普通客户`，`项目来源` 使用 `已有项目 / 普通项目`。技术标必须确认 `投标机型`，候选由素材库投标机型参数表动态解析；只有用户选中的机型参数会随项目保存并传给 `S3 缺口处理` 和 `S4 生成标书`。
